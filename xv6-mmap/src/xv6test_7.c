...
/*
Test that we split unmapped large region into two regions, but then needed to allocate on 3rd call because
the split region is too small.
*/
...
void test() {
  ...
  void* res3=0;
  res3 = mmap(res3, 0x2000, 0/*prot*/, 0/*flags*/, -1/*fd*/, 0/*offset*/);
  if (res2<=0)
  {
    printf(1, "XV6_TEST_OUTPUT : mmap failed\n");
    return;
  ...
  if ((uint)res2 != 0x3000 || (uint)res3 != 0x5000) {
